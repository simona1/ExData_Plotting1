### Code for producing "plot 4", project 1, in the "Exploratory Research" Coursera class
...
### Set the parameter values for multiple plots.  
par(mfrow=c(2,2))

# Plot1 
plot(x=selection$datetime, y=gap, xlab="", cex.lab=0.7, ylab="Global Active Power (kilowatts)", type="l")

# Plot2
plot(x=selection$datetime, y=selection$Voltage, cex.lab=0.7, xlab="datetime", ylab="Voltage", type="l")

# Plot3
plot3 <- plot(x=selection$datetime, s1, col="black", type="l", cex.lab=0.7, ylim=range(s1), ylab= "Energy sub metering", xlab="")
par(new=TRUE)
plot3 <- plot(x=selection$datetime, s2, col="red", xaxt="n", yaxt="n", type="l", cex.lab=0.7, ylim=range(s1), ylab= "Energy sub metering", xlab="")
par(new=TRUE)
plot3 <- plot(x=selection$datetime, s3, col="blue",  xaxt="n", yaxt="n", type="l", cex.lab=0.7, ylim=range(s1), ylab= "Energy sub metering", xlab="")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=c(1,1), lwd=c(2.5,2.5),col=c("black","red", "blue"), cex=0.5)

# Plot4  
plot(x=selection$datetime, y=selection$Global_reactive_power, cex.lab=0.7, xlab="datetime", ylab="Global_reactive_power", type="l")
